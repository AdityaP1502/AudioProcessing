...
class ReverbBox(CascadedBox):
  def __init__(self, delay : list[int], gain : list[float], fs : int, chunks : int, pre_delay=None):
    super().__init__()
    ...
    # initiate components
    _fir_filter_component = FIRFilter(fs=fs, chunks=chunks, wc=8000, n_order=128)
    ...
    if pre_delay != None:
      _pre_delay_comps = Delay(delay=pre_delay / 1000, fs=fs, chunks=chunks)
      for (_, _comb) in _comb_components:
        _comb.insert_input_node(_pre_delay_comps.get_output_node())
        
      super().insert_components(*_all_pass_components, (8, _fir_filter_component), *_comb_components, (4, _adder_components), (-1, _pre_delay_comps))

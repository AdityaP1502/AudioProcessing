...
class FIRFilter(BlackBox):
  ...
  def _create_taps(self):
    self._taps = firwin(self._num_taps, self._cutoff_freq, fs=self._sample_rate, pass_zero=True)
  ...
  def filter(self):
    ...
    assert len(out) == self._chunks, "Output length must equal to the chunk length. Received {}".format(len(out))

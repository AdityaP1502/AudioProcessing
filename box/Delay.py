...
class Delay(BlackBox):
  ...
  def _insert_into_delay_line(self, xn):
    assert len(xn) == self._chunks, "Input length must be equal to the chunk length of {}. Got {}".format(self._chunks, len(xn))
  ...
  def filter(self) -> np.ndarray:
    # return the sample stored in the memory and insert the new sample into the memory
